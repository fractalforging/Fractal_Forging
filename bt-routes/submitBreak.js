  ...
  router.post("/", async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        logger.info(`${kleur.magenta(user)} submitted a break of ${breakDuration} minutes. ${breakTracker.status === 'queued' ? 'Break added to the queue' : ''}`, { username: req.user.username });
        io.emit('reload');
